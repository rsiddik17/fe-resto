  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Pastikan response ada dan punya properti data
        if (response && response.data) {
          const dataMapping = response.data.map((s: any) => ({
            ...
            // Mapping Role agar tampil rapi di UI
            role:
              s.role === "CASHIER"
                ? "Kasir"
                : s.role === "KITCHEN"
                      ...
                      >
                        ...
                        <td className="py-3 text-center relative">
                          <ActionMenu
                            onDetail={() =>
                              navigate(
                                `/admin/manajemen-pegawai/detail/${pegawai.id}`,
                              )
                            }
                            onEditProfil={() => {
                              console.log("Mengirim data edit:", pegawai);
                              navigate(
                                `/admin/manajemen-pegawai/edit/${pegawai.id}`,
                                {
                            ...
                            onUbahPassword={() =>
                              navigate(
                                `/admin/manajemen-pegawai/ubah-sandi/${pegawai.id}`,
                              )
                            }
                            onHapus={() => {
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 text-[12px] font-bold text-gray-500">
                    ...
                    <div className="relative">
                      ...
                      {isDropdownOpen && (
                        <div className="absolute left-0 top-full mt-1 w-24 bg-white border rounded-md shadow-lg z-50">
                          {[5, 10, 15, 20].map((n) => (
                            <button
                              key={n}
                              onClick={() => {
          ...
          <div className="md:hidden space-y-3 pb-20">
            {currentItems.map((pegawai, index) => (
              ...
              >
                ...
                <p className="font-semibold text-black-800 text-base">
                  {pegawai.nama}
                </p>
                <p className="text-xs text-black-500 mt-1 break-all">
                  {pegawai.email}
                </p>
                ...
                <div className="mt-3 pt-2 border-t border-gray-100 flex gap-2">
                  <button
                    onClick={() =>
                      navigate(`/admin/manajemen-pegawai/detail/${pegawai.id}`)
                    }
                    className="flex-1 py-1.5 text-center text-xs font-medium bg-gray-100 text-gray-700 rounded-md"
                  >
                    Detail
                  </button>
                  <button
                    ...
                    onClick={() => {
                       console.log("Mengirim data edit (mobile):", pegawai);
                      navigate(`/admin/manajemen-pegawai/edit/${pegawai.id}`, {
                        state: { pegawaiData: pegawai },
                      });
                    }}
                    className="flex-1 py-1.5 text-center text-xs font-medium bg-gray-100 text-gray-700 rounded-md"
                  ...
                  <button
                    ...
                    className="flex-1 py-1.5 text-center text-xs font-medium bg-red-50 text-red-600 rounded-md"
                  >
                    Hapus
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-[11px] font-bold text-gray-500">
                    ...
                    <div className="relative">
                      <button
                        onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                        className="border rounded-md px-2 py-1 flex items-center gap-1 text-[11px]"
                      >
                        {itemsPerPage} Data
                        <ChevronDown
                          size={12}
                      ...
                      {isDropdownOpen && (
                        <div className="absolute left-0 bottom-full mb-1 w-20 bg-white border rounded-md shadow-lg">
                          {[5, 10, 15, 20].map((n) => (
                            <button
                              key={n}
                              onClick={() => {
                  ...
                  <span className="text-[10px] text-gray-400">
                    {startCount}-{endCount} dari {sortedPegawai.length}
                  </span>
                </div>
