import { useState, useEffect, useCallback, useMemo } from "react"; // 👈 TAMBAH useCallback, useMemo
...
const LaporanBulananPage = () => {
  ...
  useEffect(() => {
    setSortedPesanan(filteredPesanan);
    setSortedPendapatan(filteredPendapatan);
  }, [filteredPesanan, filteredPendapatan]);

  // ✅ PERBAIKAN 3: Pakai useCallback untuk periodeText
  ...
  // ✅ PERBAIKAN 4: Pakai useCallback untuk handler filter
  const handleFilterChange = useCallback((bulanList: string[], thn: string) => {
    setSelectedBulans(bulanList);
    setTahun(thn);
  }, []);

  // ✅ PERBAIKAN 5: Pakai useCallback untuk handler sort
  const handleSortPesanan = useCallback((newSortedData: PesananBulanan[]) => {
