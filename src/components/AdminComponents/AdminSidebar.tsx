...
const AdminSidebar = ({ onLogout }: AdminSidebarProps) => {
  ...
  const [isCollapsed, setIsCollapsed] = useState(() => {
    if (typeof window !== "undefined") {
      return window.innerWidth < 768; // mobile = collapsed
    }
    return false;
  });
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="absolute -right-3 top-10 bg-white w-6 h-6 rounded-full flex items-center justify-center shadow-md hover:bg-gray-100 transition-colors z-50 cursor-pointer md:flex"
        >
          {isCollapsed ? (
        ...
        >
          ...
          <span
            className={cn(
              "text-white font-bold text-xl truncate transition-all duration-300",
              isCollapsed ? "max-w-0 opacity-0" : "max-w-50 opacity-100 ml-4",
            )}
          >
            IT'S RESTO
          </span>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isCollapsed
                    ? "w-12 justify-center rounded-2xl"
                    : "w-60 px-3 rounded-2xl",
                  isActive
