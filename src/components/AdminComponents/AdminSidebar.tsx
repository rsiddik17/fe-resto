...
const AdminSidebar = ({ onLogout }: { onLogout?: () => void }) => {
  ...
  return (
    <>
      {/* ✅ HAMBURGER - HILANG SAAT SIDEBAR TERBUKA */}
      {!isMobileOpen && (
        <button
          onClick={() => setIsMobileOpen(true)}
          className="lg:hidden fixed top-4 left-4 z-50 py-2 px-2.5 flex gap-2 bg-primary text-white rounded-sm shadow-md cursor-pointer hover:bg-primary/90 transition-colors"
        >
