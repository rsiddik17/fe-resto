    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => eksporKePDFPesananMingguan(sortedData, periode)} className="bg-primary text-white font-bold text-[11px] sm:text-[12.5px] px-3 py-1.5 sm:px-5 sm:py-2 rounded-xs flex items-center gap-1 shadow-md hover:bg-primary/90">
            <ExportIcon size={10} /> Ekspor PDF
          </button>
          <button onClick={() => eksporKeExcelPesananMingguan(sortedData, periode)} className="bg-primary text-white font-bold text-[11px] sm:text-[12.5px] px-3 py-1.5 sm:px-5 sm:py-2 rounded-xs flex items-center gap-1 shadow-md hover:bg-primary/90">
      ...
      <div className="md:hidden">
        <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
          <span className="text-xs font-bold text-black-500 block mb-2">Urutkan berdasarkan:</span>
          ...
          <table className="w-full min-w-150 text-left text-[12.5px]">
            <thead className="bg-gray-200 text-gray-500 font-bold uppercase text-[11px]">
              <tr>
                ...
                <th className="py-3 px-4 cursor-pointer " onClick={() => handleSort("cancel")}>
        ...
        {currentItems.map((item, index) => (
          <div key={item.id} className="bg-white rounded-lg border border-gray-100 p-3 shadow-sm">
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between"><span className="text-black-400 font-bold">Minggu</span><span className="font-medium">{item.minggu}</span></div>
              <div className="flex justify-between"><span className="text-black-400 font-bold">Total Pesanan</span><span className="font-medium">{item.totalPesanan.toLocaleString("id-ID")}</span></div>
              <div className="flex justify-between"><span className="text-black-400 font-bold">Selesai</span><span className="font-medium text-black-600">{item.selesai.toLocaleString("id-ID")}</span></div>
              <div className="flex justify-between"><span className="text-black-400 font-bold">Cancel</span><span className="font-medium text-red-500">{item.cancel}</span></div>
            </div>
          </div>
        ))}
        <div className="bg-gray-100 rounded-lg p-3">
          <div className="flex justify-between text-xs font-bold"><span>Total</span></div>
          <div className="flex justify-between text-xs mt-1"><span>Total Pesanan:</span><span>{totalPesanan.toLocaleString("id-ID")}</span></div>
          <div className="flex justify-between text-xs"><span>Selesai:</span><span className="text-green-600">{totalSelesai.toLocaleString("id-ID")}</span></div>
          <div className="flex justify-between text-xs"><span>Cancel:</span><span className="text-red-500">{totalCancel}</span></div>
        </div>
        {enablePagination && totalPages > 1 && (
