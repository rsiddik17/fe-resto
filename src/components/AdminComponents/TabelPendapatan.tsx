    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => eksporKePDFPendapatan(sortedData, periode)} className="bg-primary text-white font-bold text-[11px] sm:text-[12.5px] px-3 py-1.5 sm:px-5 sm:py-2 rounded-xs flex items-center gap-1 shadow-md hover:bg-primary/90">
            <ExportIcon size={10} /> Ekspor PDF
          </button>
          <button onClick={() => eksporKeExcelPendapatan(sortedData, periode)} className="bg-primary text-white font-bold text-[11px] sm:text-[12.5px] px-3 py-1.5 sm:px-5 sm:py-2 rounded-xs flex items-center gap-1 shadow-md hover:bg-primary/90">
          ...
          <table className="w-full min-w-125 text-left text-[12.5px]">
            <thead className="bg-gray-200 text-gray-500 font-bold uppercase text-[11px]">
              <tr>
                ...
                <th className="py-3 px-4 cursor-pointer select-none hover:bg-gray-200" onClick={() => handleSort("tanggal")}>
                  <div className="flex items-center gap-1">TANGGAL {renderSortIcon("tanggal")}</div>
                </th>
                <th className="py-3 px-4 cursor-pointer select-none hover:bg-gray-200" onClick={() => handleSort("totalPesanan")}>
                  <div className="flex items-center gap-1">TOTAL PESANAN {renderSortIcon("totalPesanan")}</div>
                </th>
                <th className="py-3 px-4 cursor-pointer select-none hover:bg-gray-200" onClick={() => handleSort("pendapatan")}>
          ...
          <div className="flex items-center justify-between py-3 px-4 border-t border-gray-100 bg-white">
            ...
            <div className="flex items-center gap-1">
              ...
              {Array.from({length: totalPages}, (_, i) => i+1).map(p => (
                <button key={p} onClick={() => setCurrentPage(p)} className={`w-7 h-7 rounded border ${currentPage === p ? "bg-primary text-white border-primary" : "border-gray-200"}`}>{p}</button>
              ))}
      ...
      <div className="md:hidden space-y-3">
          ...
          <div key={item.id} className="bg-white rounded-lg border border-gray-100 p-3 shadow-sm">
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-800">{item.totalPesanan.toLocaleString("id-ID")}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-black-400 font-bold">Total Pendapatan</span>
                <span className="font-bold text-primary">Rp {item.pendapatan.toLocaleString("id-ID")}</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="bg-gray-100 rounded-lg border border-gray-200 p-3">
          <div className="flex justify-between text-xs font-bold">
            <span className="text-black-600 font-bold">Total </span>
            <span className="text-primary">Rp {totalPendapatan.toLocaleString("id-ID")}</span>
          </div>
          <div className="flex justify-between text-xs text-black-500 mt-1">
            <span>Total Pesanan:</span>
            <span>{totalPesanan.toLocaleString("id-ID")}</span>
          </div>
        </div>
