...
export default function TabelPendapatanMingguan({
  ...
  const [sortField, setSortField] = useState<"minggu" | "totalPesanan" | "pendapatan" | "">("");
          ...
          <table className="w-full min-w-125 text-left text-[12.5px]">
            <thead className="bg-gray-200 text-gray-500 font-bold uppercase text-[11px]">
              <tr>
                ...
                <th className="py-3 px-4 cursor-pointer " onClick={() => handleSort("pendapatan")}>
            ...
            <tbody className="font-medium text-gray-800">
              {currentItems.map((item, index) => (
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 font-bold">Rp {item.pendapatan.toLocaleString("id-ID")}</td>
                </tr>
              ))}
        ...
        {currentItems.map((item, index) => (
          <div key={item.id} className="bg-white rounded-lg border border-gray-100 p-3 shadow-sm">
            ...
            <div className="space-y-1.5 text-xs">
              ...
              <div className="flex justify-between"><span className="text-black-400 font-bold">Pendapatan</span><span className="font-bold text-primary">Rp {item.pendapatan.toLocaleString("id-ID")}</span></div>
            </div>
          </div>
        ))}
        <div className="bg-gray-100 rounded-lg p-3">
          <div className="flex justify-between text-xs font-bold"><span>Total </span><span>Rp {totalPendapatan.toLocaleString("id-ID")}</span></div>
