...
const WaiterOrderSummary = ({ items, subTotal, taxRate = 10, discountAmount = 0, onAddDiscount, onRemoveDiscount }: WaiterOrderSummaryProps) => {
  ...
  return (
    <div className="bg-white border-[1.5px] border-gray-200 rounded-md p-3 shadow-sm flex flex-col gap-2 animate-in fade-in duration-300">
      ...
      <div className="mt-2">
        {discountAmount > 0 ? (
          <Button variant="outline" onClick={onRemoveDiscount} className="w-full py-1.5 text-[14px] font-bold border-[1.5px] border-gray-300 text-gray-500 hover:bg-gray-50 rounded-md">
            Batalkan Diskon
          </Button>
        ) : (
      ...
      {/* Total Akhir */}
      <div className="flex justify-between items-center">
        <span className="font-extrabold text-[14.5px] text-black">Total Pembayaran</span>
        <span className="font-extrabold text-[14.5px] text-black">{rupiahFormatter.format(grandTotal)}</span>
      </div>
