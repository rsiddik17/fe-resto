...
const DeleteConfirmModal = ({ onClose, onConfirm }: DeleteConfirmModalProps) => {
  return (
    ...
    >
      <div 
        className="bg-white w-full max-w-131.5 h-91 rounded-2xl pt-9 pb-8 px-8 shadow-sm flex flex-col items-center text-center animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Ikon Segitiga Peringatan */}
        <div className="mb-4">
          <WarningIcon className="text-[#FC1111] w-12 h-12" />
        </div>

        <h2 className="text-[26px] font-bold text-[#FC1111] mb-4.5">
          Hapus item ini dari keranjang?
        </h2>
        
        <p className="mb-9 text-xl px-2">
        ...
        <div className="flex justify-center w-full gap-x-16">
          <Button 
            ...
            className="flex-1 max-w-41.75 font-normal text-lg bg-[#D9D9D9] text-black"
          >
            Batal
          </Button>
