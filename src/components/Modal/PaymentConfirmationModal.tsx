  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/3 backdrop-blur-[3px] p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-87.5 rounded-sm shadow-sm overflow-hidden animate-in zoom-in-95 duration-200 relative p-6 pt-8 flex flex-col items-center text-center" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Ikon Centang Ungu */}
        <div className="bg-primary w-20 h-20 rounded-full flex items-center justify-center text-white mb-8 shadow-sm">
        ...
        <div className="flex items-center gap-4 w-[97%]">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 bg-primary text-white text-[15px] font-bold py-2.5 rounded-xs hover:bg-[#5a0b64] transition-colors shadow-sm cursor-pointer"
          >
            Validasi
          </button>
        </div>
