...
const IncomingOrderTable = ({ orders }: IncomingOrderTableProps) => {
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
      {/* Header Tabel */}
      <div className="flex justify-between items-center pl-7 pr-6 py-5 shrink-0">
        ...
        <table className="w-full text-left border-collapse min-w-150">
          <thead>
            <tr className="bg-[#EFEEEE] border-gray-100 text-[#73736C] text-[11px] md:text-[12.5px] uppercase">
              <th className="py-2 pl-7 pr-1 font-bold tracking-wider w-[16%]">ORDER ID</th>
              <th className="py-2 px-4 font-bold tracking-wider w-[24%]">MENU</th>
              <th className="py-2 px-4 font-bold tracking-wider w-[10%]">MEJA</th>
              <th className="py-2 px-4 font-bold tracking-wider w-[15%]">STATUS</th>
              <th className="py-2 px-3 font-bold tracking-wider w-[10%]">TOTAL</th>
              <th className="py-2 pr-7 pl-1 font-bold tracking-wider w-[25%] text-center">METODE PESAN</th>
            </tr>
          </thead>
          <tbody className="text-[13px] md:text-[14px]">
            {orders.map((order) => (
              ...
              >
                <td className="pt-0 pb-4 pl-7 pr-5 font-bold">{order.id}</td>
                <td className="pt-1.5 pb-4 pl-4">
                  <div className="wrap-break-word leading-tight">{order.menu}</div>
                </td>
                <td className="pt-1.5 pb-4 px-4 text-black">{order.table}</td>
                <td className="pt-1.5 pb-4 px-3">
                  <span className="bg-orange text-white px-2.5 py-0.75 rounded-full text-[12px] font-bold uppercase shadow-sm">
                    {order.status}
                  </span>
                </td>
                <td className="pt-1.5 pb-4 px-3 font-bold text-[13.5px]">
                  {rupiahFormatter.format(order.total)}
                </td>
                <td className="pt-1.5 pb-4 pr-7 pl-0 text-center">
                  <span
                    className={cn(
