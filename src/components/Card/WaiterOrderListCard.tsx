...
const WaiterOrderListCard = ({
  ...
  return (
    <div className="bg-white rounded-md shadow-sm border border-gray-100 border-l-11 border-l-primary relative overflow-hidden flex flex-col w-full py-5 px-3.5">
      ...
      <div
        className={`absolute top-0 right-0 ${badgeColor} text-white px-3 py-0.5 rounded-bl-md font-extrabold text-[10px]`}
      >
        {status}
      </div>
      ...
      <div className="flex justify-between items-start mb-2.5">
        <div>
          <p className="text-black/50 text-[13.5px] md:text-sm font-bold mb-1.5">
            {orderId}
          </p>
          <h3 className="font-bold text-[16.5px] md:text-[17px]">
            {tableName}
          </h3>
        </div>
        <span className="text-gray-400 text-[13px] md:text-[13px] mt-5">
          {time}
        </span>
      </div>
      ...
      <div className="flex justify-between items-center border-t border-gray-500 pt-2 mt-auto mb-3 mr-3">
        ...
        <span className="text-black text-[14px] md:text-sm font-medium">
      ...
      <div className="flex gap-2 md:gap-3 mr-3">
        <Button
          ...
          className="flex-1 py-2 md:py-2.5 text-[14px] md:text-[13px] lg:text-[13px] font-normal border-[1.5px] border-primary text-primary hover:bg-primary/5 rounded-sm"
        >
          Lihat Detail
        </Button>
        {!isDimasak && (
